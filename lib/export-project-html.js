    ...
    export: async function() {
      var exporter = {
        ...
        run: async function() {
            ...
            for (var i = 0; i < html_lines.length; i++) {
              ...
              if (p > 0) {
                ...
                if (html_lines[i].indexOf("<script ") >= 0 && atom.config.get("export-project-html.download_external_scripts")) {
                  ...
                  {
                    console.log("stylesheet: " + abs_path)
                  }
          ...
          text = editor.getText();
          var html = this.getHtml(editor, title, target_path, (function(_this) {
      ...
      async function scanFolder(dir) {
        ...
        for (let i = 0; i < paths.length; i++) {
          path = paths[i];
          if (path.isDirectory()) {
            if (!atom.config.get("export-project-html.ignored_dirs").includes(path.getBaseName()) && path.getBaseName() != "WebPublish") {
            ...
            if (!ignore_this_file) {
              await atom.workspace.open(path.getPath()); // open file in atom so that it can get converted to html
              await exporter.run();
