// from the Mardown-Preview Package:
// https://github.com/atom/markdown-preview/blob/master/lib/renderer.js
...
var convertAtomEditorToStandardElement = (editorElement, preElement) => {
  return new Promise(function (resolve) {
    ...
    console.log(languageMode.grammar.scopeName)
    if (
      languageMode.fullyTokenized || languageMode.tree 
      // THE FOLLOWING LINE IS A HACK
      // For some unknown reason, when the codeblock language is plan text,
      // languageMode.fullyTokenized is not yet set to fullyTokenized,
      // but the onDidTokenize() in the else statement never kicks in.
      // Since plain text doesn't need any syntax highlighting anyway,
      // we don't have to worry about tokenisation. 
      || languageMode.grammar.scopeName == "text.plain"
    ) {
      done()
    } else {
