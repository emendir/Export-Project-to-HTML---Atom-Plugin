(function() {
  ...
  ignored_dirs= ["build", ".git", "__pycache__"];
  ignored_exts= [".pyc", ".gcode", ".png", ".jpg", ".html", ".blend", ".stl"];
  ...
  module.exports = ExportHtml = {
    ...
    export: function() {
      ...
      atom.workspace.onDidOpen(function(event) {
        var exporter = {
          ...
          run: function() {
            console.log("enter function run")
            os = require("os");
            path = require("path");
            exec = require('child_process').exec;
            Shell = require('shell');
    ...
    scanFolder: function(dir){
      ...
      for (let i = 0; i < paths.length; i++) {
        ...
        if(path.isDirectory() && !ignored_dirs.includes(path.getBaseName()))
        {
          this.scanFolder(path);
        }
        else {
          if(!ignored_exts.includes(path.getBaseName())){
            atom.workspace.open(path.getPath());
          }
        }
