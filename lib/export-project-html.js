        ...
        run: async function() {
          ...
          if (this.isMarkdownEditor(atom.workspace.getActiveTextEditor())) {
            ...
            for (var i = 0; i < html_lines.length; i++) {
              ...
              if (p > 0) {
                var abs_path = html_lines[i].substring(p + 4)
                p = abs_path.indexOf("\"")
                abs_path = abs_path.substring(p + 1, abs_path.length)
                abs_path = abs_path.substring(0, abs_path.indexOf("\""))
                ...
                if (abs_path.substring(0, atom.project.getDirectories()[0].getPath().length) == atom.project.getDirectories()[0].getPath()) {
                  rel_path = abs_path.substring(atom.project.getDirectories()[0].getPath().length + 1)
                ...
                var extension = abs_path.split(".").slice(-1)[0]
                if (extension == "md" || extension in atom.config.get("export-project-html.file_types_to_convert_to_html")) {
                  ...
                  _path = _path + ".html"
                  console.log(_path)
                  html_lines[i] = html_lines[i].replace(abs_path, _path)
                } else {
