(function() {
  ...
  downloadFile = require('./file-downloader');
  ...
  async function processMarkdownEditor(target_path) {
    ...
    for (var i = 0; i < html_lines.length; i++) { // for each line of HTML code
      var url_property_index = Math.max(html_lines[i].indexOf("src="), html_lines[i].indexOf("href="))
      ...
      if (url_property_index > 0) {
        ...
        half_trimmed_line = half_trimmed_line.substring(half_trimmed_line.indexOf("=") + 1)
          ...
          if (grammar.scopeName === "source.gfm") {
            ...
            for (var i = 0; i < html_lines.length; i++) {
              ...
              if (url_index > 0) {
                ...
                if (atom.config.get("export-project-html.download_external_scripts") && html_lines[i].indexOf("<script ") >= 0 && url_path.substring(0, 8) == "https://") {
                  ...
                  html_lines[i] = html_lines[i].replace(url_path, "/" + filename)

                }
                // if line is a reference to an https URL of an external stylesheet,
                // download that stylesheet and make a local reference
                else if (atom.config.get("export-project-html.download_external_stylesheets") && html_lines[i].indexOf("<link rel=\"stylesheet\" ") >= 0 && url_path.substring(0, 8) == "https://") {
                  var filename = String(url_path.split("/").slice(-1))
                  const filepath = path.join(atom.project.getDirectories()[0].getPath(), "WebPublish", filename)
                  if (!fs.existsSync(filepath)) {
                    downloadFile(url_path, filepath)
